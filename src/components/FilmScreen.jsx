 ...
 *   sprocket holes run right to left, the photo cuts over instantly behind a split-second
 *   black blink (a projector shutter), and a film filter - grain, scratches, a warm burn
 *   at the edges - flickers over it for a moment before clearing
 * - A click (or a tap on touch screens) advances too
 * - The cut waits until the next photo is downloaded and decoded, so the blink and filter
 *   always land on the new photo - on a slow first visit a hover could otherwise play the
 *   transition over a blank or unchanged frame
 ...
 *   shows a photo
 * - Reduced-motion users get an instant swap, with no blink or filter
...
const FILM_BLINK_MS = 70;         // the shutter blink as the photo cuts over
const FILM_FILTER_MS = 480;       // the film filter flickering over the new photo

// Film grain: fractal noise as a small tiling SVG, darkened to a speckle with some transparency
const FILM_GRAIN = `url("data:image/svg+xml,${encodeURIComponent(
...
// Two hairline scratches running down the frame, one light and one dark
const FILM_SCRATCHES = 'linear-gradient(90deg, transparent 23%, rgba(255, 255, 255, 0.6) 23.2%, transparent 23.5%, transparent 71%, rgba(0, 0, 0, 0.35) 71.15%, transparent 71.4%)';
// Warm light in the middle burning to brown at the edges
const FILM_BURN = 'radial-gradient(ellipse at center, rgba(255, 214, 160, 0.14) 0%, rgba(60, 30, 5, 0.45) 100%)';
...
const injectFilmStyles = (() => {
  ...
  return () => {
    ...
    style.textContent = `
      /* Shutter: black for a split second as the photo cuts over, then gone */
      @keyframes filmBlink {
        from { opacity: 1; }
        to   { opacity: 0; }
      }
      /* Film filter: flickers unevenly over the new photo, then clears */
      @keyframes filmFilterFlicker {
        0%   { opacity: 0; }
        10%  { opacity: 0.9; }
        20%  { opacity: 0.35; }
        30%  { opacity: 0.8; }
      ...
      /* Grain jumps between positions so it crawls like real film */
      @keyframes filmGrainJitter {
        0%   { background-position: 0 0, 0 0, 0 0; }
        25%  { background-position: 0 0, 0 0, 37px -21px; }
        50%  { background-position: 3px 0, 0 0, -54px 43px; }
        75%  { background-position: -2px 0, 0 0, 18px 67px; }
      }
      ...
      @keyframes filmLeaderRun {
        ...
        to   { background-position-x: calc(100% - var(--film-travel, 44px)); }
      }
      .film-leader-run { animation: filmLeaderRun ${FILM_ADVANCE_MS}ms cubic-bezier(0.22, 0.61, 0.36, 1); }
      @media (max-width: 768px) {
        .film-leader { display: none; }
      }
      .film-blink { animation: filmBlink ${FILM_BLINK_MS}ms steps(1, end) both; }
      .film-filter {
        animation: filmFilterFlicker ${FILM_FILTER_MS}ms steps(1, end) both,
                   filmGrainJitter 120ms steps(1, end) 4;
      }
      ...
      @media (prefers-reduced-motion: reduce) {
        ...
        .film-blink, .film-filter { display: none; }
      }
    `;
  ...
  >
    {['top', 'bottom'].map((edge) => (
      <div
        ...
        style={{ ...FILM_SPROCKET_ROW, position: 'absolute', left: 0, right: 0, [edge]: '10px', backgroundPositionX: '100%' }}
      />
    ))}
  </div>
  );
...
const FilmScreen = ({ images = [], keepOuts = [] }) => {
  ...
  const advance = () => {
    ...
    // If the next photo is still loading, hold the cut until it's ready (or has failed)
    waiting.current = true;
    (nextImage?.decode ? nextImage.decode() : Promise.resolve()).then(cut, cut);
  };
  ...
  return (
    ...
    >
      ...
      {/* The gate: the photo cuts over instantly; the shutter blink and film filter play on top */}
      <div
        ref={gateRef}
        className="film-gate"
        style={{
              ...
              <>
                <div
                  ...
                  style={{
                    ...
                    inset: 0,
                    pointerEvents: 'none',
                    backgroundImage: `${FILM_SCRATCHES}, ${FILM_BURN}, ${FILM_GRAIN}`,
                    backgroundSize: '100% 100%, 100% 100%, 160px 160px',
                  }}
                />
                <div
                  key={`blink-${advances}`}
                  className="film-blink"
                  aria-hidden="true"
